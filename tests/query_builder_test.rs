...
fn test_basic_query_creation() {
    ...
    assert_eq!(query.len(), all_blocks.len(), "Two AllBlocks should return same number of blocks");
}
...
fn test_block_filtering() {
    ...
    assert!(solid_query.len() <= original_count, "Solid filter should not increase block count");
    
    // Test survival blocks filter
    let survival_query = AllBlocks::new().survival_only();
    assert!(survival_query.len() <= original_count, "Survival filter should not increase block count");
    
    // Test chaining filters
    let chained_query = AllBlocks::new()
        .only_solid()
        .survival_only();
    assert!(chained_query.len() <= solid_query.len(), "Chained filters should be more restrictive");
}
...
fn test_property_filtering() {
    ...
    assert!(with_delay.len() > 0, "Should find blocks with delay property");
    ...
    let multiple_props = query.clone()
        .with_property("delay")
        .with_property("facing");
    assert!(multiple_props.len() > 0, "Should find blocks with both properties");
}
...
fn test_color_filtering() {
    ...
    for block in with_color.collect() {
        assert!(block.extras.color.is_some(), "Block {} should have color data", block.id());
    }
}
...
fn test_pattern_matching() {
    ...
    assert!(stone_blocks.len() > 0, "Should find blocks with 'stone' in name");
    ...
    assert!(multi_pattern.len() > 0, "Should find blocks matching pattern");
}
...
fn test_block_families() {
    ...
    assert!(multi_family.len() >= stairs.len(), "Multiple families should include stairs");
}
...
fn test_exclusion_filters() {
    ...
    assert!(no_transparent.len() <= original_count, "Excluding transparent should not increase count");
    
    // Test excluding tile entities
    let no_tile_entities = query.clone().exclude_tile_entities();
    assert!(no_tile_entities.len() <= original_count, "Excluding tile entities should not increase count");
    
    // Test excluding falling blocks
    let no_falling = query.clone().exclude_falling();
    assert!(no_falling.len() <= original_count, "Excluding falling blocks should not increase count");
}
...
fn test_sorting() {
    ...
    assert_eq!(name_sorted.len(), query.len(), "Sorting should preserve count");
    
    // Test sorting by color gradient
    let color_sorted = query.clone().sort_by_color_gradient();
    assert_eq!(color_sorted.len(), query.len(), "Sorting should preserve count");
}
...
fn test_color_similarity() {
    ...
    if query.len() > 0 {
        ...
        assert!(similar.len() <= query.len(), "Color similarity should not increase count");
        
        // Test sorting by color similarity
        let sorted_by_similarity = query.clone().sort_by_color_similarity(target_color);
        assert_eq!(sorted_by_similarity.len(), query.len(), "Sorting should preserve count");
    }
}
...
fn test_limit_and_offset() {
    ...
    assert!(limited.len() <= original_count, "Limit should not increase count");
...
fn test_gradient_generation() {
    ...
    if query.len() >= 3 {
        ...
        assert_eq!(gradient.len(), 5, "Gradient should have requested number of steps");
...
fn test_gradient_between_colors() {
    ...
    if query.len() >= 2 {
        ...
        let config = GradientConfig::new(5)
            .with_color_space(ColorSpace::Rgb)
            .with_easing(EasingFunction::Linear);
        let gradient = query.clone().generate_gradient_between_colors(
            start_color, 
            end_color, 
            config
        );
        
        assert_eq!(gradient.len(), 5, "Gradient between colors should have requested steps");
    }
}
...
fn test_multi_gradient() {
    ...
    assert!(gradient.len() <= 6, "Multi-gradient should not exceed requested steps");
    ...
    if query.len() > 0 {
        ...
        println!("Multi-gradient returned {} blocks from {} colored input blocks", gradient.len(), query.len());
    }
}
...
fn test_query_chaining() {
    ...
    assert!(complex_query.len() <= 10, "Complex query should respect limit");
    ...
    if complex_query.len() >= 3 {
        ...
        assert_eq!(gradient_query.len(), 3, "Should be able to chain after gradient");
    }
}
...
fn test_empty_query_handling() {
    ...
    assert_eq!(empty_query.len(), 0, "Impossible pattern should return no blocks");
    ...
    assert_eq!(empty_gradient.len(), 0, "Gradient of empty query should be empty");
    
    let empty_limited = empty_query.limit(10);
    assert_eq!(empty_limited.len(), 0, "Limit on empty query should still be empty");
}
...
fn test_query_builder_robustness() {
    ...
    assert_eq!(invalid_prop.len(), 0, "Invalid property should return no blocks");
    
    // Test with invalid property values
    let invalid_value = query.clone().with_property_value("delay", "999");
    assert_eq!(invalid_value.len(), 0, "Invalid property value should return no blocks");
    ...
    assert_eq!(huge_limit.len(), query.len(), "Huge limit should return all blocks");
}
