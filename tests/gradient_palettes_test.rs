use blockpedia::color::palettes::{PaletteGenerator, ColorGradient, GradientMethod};
...
fn test_basic_gradient_generation() {
    ...
    let gradient = PaletteGenerator::generate_gradient_palette(
        red, blue, 5, GradientMethod::LinearRgb
    );
...
fn test_multi_color_gradient() {
    ...
    let gradient = PaletteGenerator::generate_multi_gradient_palette(
        vec![red, green, blue], 7, GradientMethod::LinearRgb
    );
    ...
    assert!(has_green_influence, "Gradient should show green influence in middle");
}
...
fn test_themed_palettes() {
    ...
    let sunset_avg_temp = sunset.iter()
        .map(|c| c.rgb[0] as u32 + c.rgb[1] as u32)
        .sum::<u32>() / (sunset.len() as u32 * 2);
    
    // Ocean should have cool colors (more blue)
    let ocean_avg_blue = ocean.iter()
        .map(|c| c.rgb[2] as u32)
        .sum::<u32>() / ocean.len() as u32;
    
    // Fire should be warm (high red/yellow)
    let fire_avg_warm = fire.iter()
        .map(|c| c.rgb[0] as u32 + c.rgb[1] as u32)
        .sum::<u32>() / (fire.len() as u32 * 2);
...
fn test_monochrome_palette() {
    ...
    assert!(first_brightness < last_brightness, "Monochrome should go from dark to light");
    ...
    assert!(has_base_color, "Monochrome should include a color reasonably close to the base color");
}
...
fn test_hue_interpolation() {
    ...
    let gradient = PaletteGenerator::generate_gradient_palette(
        red, purple, 5, GradientMethod::LinearHsl
    );
...
fn test_edge_cases() {
    ...
    let single = PaletteGenerator::generate_gradient_palette(
        color, color, 3, GradientMethod::LinearRgb
    );
...
fn test_gradient_smoothness() {
    ...
    let gradient = PaletteGenerator::generate_gradient_palette(
        start, end, 10, GradientMethod::LinearRgb
    );
...
fn test_complementary_colors() {
    ...
    let unique_colors: std::collections::HashSet<_> = complementary.iter()
        .map(|c| c.rgb)
        .collect();
    ...
    fn test_real_block_gradient() {
        ...
        if colored_blocks.len() >= 2 {
            ...
            let gradient = PaletteGenerator::generate_block_gradient_palette(
                color1, color2, 8, GradientMethod::LinearOklab
            );
            ...
            let start_distance = ((gradient[0].rgb[0] as i32 - color1.rgb[0] as i32).abs() +
                                 ...
                                 (gradient[0].rgb[2] as i32 - color1.rgb[2] as i32).abs()) as f32;
            let end_distance = ((gradient[7].rgb[0] as i32 - color2.rgb[0] as i32).abs() +
                               (gradient[7].rgb[1] as i32 - color2.rgb[1] as i32).abs() +
                               (gradient[7].rgb[2] as i32 - color2.rgb[2] as i32).abs()) as f32;
            assert!(start_distance < 50.0, "Start color should be reasonably close to original: distance = {}", start_distance);
            assert!(end_distance < 50.0, "End color should be reasonably close to original: distance = {}", end_distance);
        }
    }
