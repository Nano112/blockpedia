...
fn test_direction_apply_rotation() {
    ...
    assert_eq!(north.apply_rotation(Rotation::Clockwise270), Direction::West);
}
...
fn test_rotate_simple_block() -> Result<()> {
    ...
    let repeater = BlockState::parse("minecraft:repeater[delay=2,facing=north,locked=true,powered=true]")?;
...
fn test_rotate_180_degrees() -> Result<()> {
    let stairs = BlockState::parse("minecraft:oak_stairs[facing=north,half=bottom,shape=straight]")?;
...
fn test_rotate_full_circle() -> Result<()> {
    let original = BlockState::parse("minecraft:oak_stairs[facing=north,half=bottom,shape=straight]")?;
...
fn test_material_variant() -> Result<()> {
    ...
    let oak_stairs = BlockState::parse("minecraft:oak_stairs[facing=north,half=bottom,shape=straight]")?;
...
fn test_complex_rotation_with_material_change() -> Result<()> {
    ...
    let original = BlockState::parse("minecraft:oak_stairs[facing=west,half=top,shape=inner_left,waterlogged=false]")?;
...
fn test_stair_shape_rotation() -> Result<()> {
    ...
    let stairs = BlockState::parse("minecraft:oak_stairs[facing=north,half=bottom,shape=inner_left]")?;
...
fn test_material_and_shape_extraction() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        ("minecraft:cobblestone_wall", "cobblestone", BlockShape::Wall),
        ...
        ("minecraft:stone_pressure_plate", "stone", BlockShape::PressurePlate),
    ...
    for (block_id, expected_material, expected_shape) in test_cases {
        if let Some(_block) = get_block(block_id) {
            ...
            if let Ok(variants) = state.available_shapes() {
                println!("{} has {} shape variants: {:?}", block_id, variants.len(), variants);
            ...
            if let Ok(materials) = state.available_materials() {
                println!("{} has {} material variants: {:?}", block_id, materials.len(), materials);
                if materials.len() > 1 {
