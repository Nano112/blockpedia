...
fn test_gradient_edge_cases() {
    ...
    assert_eq!(
        empty_gradient.len(),
        0,
        "Empty query should produce empty gradient"
    );
    ...
    let single_block_gradient = AllBlocks::new()
        ...
        .generate_gradient(GradientConfig::new(5));
    assert!(
        single_block_gradient.len() <= 1,
        "Single block should not produce more than 1 result"
    );
    ...
    assert!(
        large_gradient.len() <= 1000,
        "Large gradient should not exceed requested size"
    );
}
...
fn test_multi_gradient_comprehensive() {
    ...
    if colored_count > 0 {
        assert!(
            multi_gradient.len() <= 10,
            "Multi-gradient should respect size limit"
        );
        ...
        for block in multi_gradient.collect() {
            assert!(
                block.extras.color.is_some(),
                "Multi-gradient block {} should have color data",
                block.id()
            );
        }
    }
...
fn test_sampling_method_consistency() {
    ...
    for &sampling in &sampling_methods {
        ...
        assert!(
            gradient.len() <= 6,
            "Gradient with {:?} sampling should respect size limit",
            sampling
        );
...
fn test_gradient_size_scaling() {
    ...
    for &size in &sizes {
        ...
        assert!(
            gradient.len() <= size,
            "Gradient size {} should not exceed requested size",
            size
        );
...
fn test_color_gradient_sort() {
    ...
    for block in &blocks {
        assert!(
            block.extras.color.is_some(),
            "Sorted block {} should have color data",
            block.id()
        );
    }
...
fn test_gradient_endpoints_match_parameters() {
    ...
    let test_cases = [
        ...
        (
            ...
            ExtendedColorData::from_rgb(255, 255, 0),
        ),
        // Purple to Orange
        (
            ExtendedColorData::from_rgb(128, 0, 128),
            ExtendedColorData::from_rgb(255, 165, 0),
        ),
    ];
...
fn test_gradient_endpoints_exact_color_matching() {
    ...
    let colored_blocks: Vec<_> = AllBlocks::new().with_color().limit(20).collect();
    ...
    if gradient_blocks.len() >= 2 {
        ...
        println!(
            "  First: {} - RGB({}, {}, {})",
            first_gradient_block.id(),
            first_gradient_color.rgb[0],
            first_gradient_color.rgb[1],
            first_gradient_color.rgb[2]
        );
        println!(
            "  Last: {} - RGB({}, {}, {})",
            last_gradient_block.id(),
            last_gradient_color.rgb[0],
            last_gradient_color.rgb[1],
            last_gradient_color.rgb[2]
        );
        ...
        assert!(
            start_distance < 1.0,
            "First gradient block should be very close to start color (distance: {:.6})",
            start_distance
        );

        assert!(
            end_distance < 1.0,
            "Last gradient block should be very close to end color (distance: {:.6})",
            end_distance
        );
