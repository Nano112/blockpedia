use blockpedia::color::block_palettes::{BlockPaletteGenerator, PaletteTheme, BlockFilter};
...
fn test_architectural_palette_generation() {
    ...
    if let Some(medieval_palette) = BlockPaletteGenerator::generate_architectural_palette("medieval") {
        ...
        let block_names: Vec<_> = medieval_palette.blocks.iter()
            .map(|rec| rec.block.id())
            .collect();
        
        assert!(block_names.iter().any(|name| name.contains("cobblestone") || name.contains("oak")));
    }
}
...
fn test_gradient_palette_with_colored_blocks() {
    ...
    if colored_blocks.len() >= 2 {
        ...
        if let Some(gradient_palette) = BlockPaletteGenerator::generate_block_gradient(block1, block2, 5) {
            ...
            let roles: Vec<_> = gradient_palette.blocks.iter()
...
fn test_monochrome_palette() {
    ...
    let colored_block = BLOCKS.values()
        .find(|b| b.extras.color.is_some());
...
fn test_palette_export_formats() {
    if let Some(palette) = BlockPaletteGenerator::generate_natural_palette("desert") {
        ...
        let _parsed: serde_json::Value = serde_json::from_str(&json_export)
...
fn test_filtered_palette_generation() {
    ...
    if let Some(filtered_forest) = BlockPaletteGenerator::generate_natural_palette_filtered("forest", &solid_filter) {
    ...
    if let Some(filtered_medieval) = BlockPaletteGenerator::generate_architectural_palette_filtered("medieval", &solid_filter) {
...
fn test_filter_impact_on_block_count() {
    ...
    let default_count = BLOCKS.values().filter(|b| default_filter.allows_block(b)).count();
