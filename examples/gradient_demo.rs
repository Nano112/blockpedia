...
fn main() {
    ...
    for (name, easing) in &easing_functions {
        ...
        for (i, block) in gradient.collect().iter().enumerate() {
            if let Some(color) = block.extras.color {
                let ext_color = color.to_extended();
                let brightness =
                    (ext_color.rgb[0] as u32 + ext_color.rgb[1] as u32 + ext_color.rgb[2] as u32)
                        / 3;
                println!(
                    "     {}: {} - Brightness: {}",
                    i + 1,
        ...
        for (i, block) in blocks.iter().enumerate() {
            if let Some(color) = block.extras.color {
                ...
                println!(
                    ...
                    block.id().replace("minecraft:", ""),
                    ext_color.hsl[0],
                    ext_color.hsl[1] * 100.0,
                    ext_color.hsl[2] * 100.0
                );
            }
        }
        println!();
    ...
    println!(
        "   Multi-color gradient: {} blocks found",
        multi_gradient.len()
    );
    ...
    println!(
        "   Color-sorted blocks: {} blocks found",
        sorted_blocks.len()
    );
