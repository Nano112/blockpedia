use blockpedia::{queries::*, BlockState, BLOCKS};
...
fn main() {
    ...
    match BlockState::new("minecraft:repeater")
        .unwrap()
        .with("invalid_prop", "value")
    {
        Ok(_) => println!("  ERROR: Should have failed!"),
    ...
    println!(
        "  ✓ Enhanced block families found: {:?}",
        enhanced_families.keys().collect::<Vec<_>>()
    );

    // Find blocks with complex properties
    let complex_blocks: Vec<_> =
        blocks_with_complex_properties(&[("delay".to_string(), vec!["2".to_string()])]).collect();
    println!(
        "  ✓ Blocks with delay=2: {:?}",
        complex_blocks.iter().map(|b| b.id()).collect::<Vec<_>>()
    );
    ...
    println!(
        "  ✓ Blocks similar to 'minecraft:repeater': {:?}",
        similar_blocks
            .iter()
            .map(|(b, count)| (b.id(), count))
            .collect::<Vec<_>>()
    );
    ...
    println!(
        ...
        advanced_stats.most_diverse_property.0, advanced_stats.most_diverse_property.1
    );
    println!(
        "    - Most correlated properties: {:?}",
        advanced_stats.most_correlated_properties
    );
    ...
    println!(
        "  ✓ Blocks with delay=1: {:?}",
        delay_blocks.iter().map(|b| b.id()).collect::<Vec<_>>()
    );

    // Find blocks matching predicate
    let redstone_blocks: Vec<_> =
        find_blocks_matching(|block| block.has_property("powered")).collect();
    println!(
        "  ✓ Blocks with 'powered' property: {:?}",
        redstone_blocks.iter().map(|b| b.id()).collect::<Vec<_>>()
    );
    ...
    println!(
        ...
        minecraft_blocks.iter().map(|b| b.id()).collect::<Vec<_>>()
    );
    ...
    println!(
        ...
        stats.total_unique_properties
    );
    println!(
        "    - Most common property: {} (appears in {} blocks)",
    ...
    println!(
        "    - Average properties per block: {:.1}",
    ...
    if let Some(closest) = blockpedia::BlockFacts::closest_to_color([128, 128, 128]) {
        println!(
            "    - Closest block to gray (128,128,128): {}",
            closest.id()
        );
    }
    ...
    println!(
        "    - Blocks with similar colors to dark gray: {:?}",
        similar_ids
    );
    ...
    match blockpedia::queries::validated::validate_block_properties_safe(
        "minecraft:repeater",
        &invalid_properties,
    ) {
        Ok(_) => println!("  ERROR: Should have failed!"),
        Err(e) => println!(
            "  ✓ Comprehensive validation caught multiple errors:\n    {}",
            e
        ),
    }
