use blockpedia::color::block_palettes::{BlockPaletteGenerator, BlockFilter};
...
fn showcase_solid_blocks_filter() {
    ...
    println!("• Exclude tile entities: {}", solid_filter.exclude_tile_entities);
    println!("• Full blocks only: {}", solid_filter.full_blocks_only);
    println!("• Exclude needs support: {}", solid_filter.exclude_needs_support);
    println!("• Exclude transparent: {}", solid_filter.exclude_transparent);
    println!("• Survival obtainable only: {}", solid_filter.survival_obtainable_only);
    println!();
    
    if let Some(medieval_solid) = BlockPaletteGenerator::generate_architectural_palette_filtered("medieval", &solid_filter) {
...
fn showcase_structural_blocks_filter() {
    ...
    println!("• Exclude falling blocks: {}", structural_filter.exclude_falling);
    println!("• Exclude tile entities: {}", structural_filter.exclude_tile_entities);
    println!("• Full blocks only: {}", structural_filter.full_blocks_only);
    println!("• Exclude needs support: {}", structural_filter.exclude_needs_support);
    println!("• Exclude transparent: {}", structural_filter.exclude_transparent);
    println!("• Exclude light sources: {}", structural_filter.exclude_light_sources);
    println!("• Exclude glass and water: Yes");
    println!();
    
    if let Some(modern_structural) = BlockPaletteGenerator::generate_architectural_palette_filtered("modern", &structural_filter) {
...
fn showcase_decorative_blocks_filter() {
    ...
    println!("• Exclude falling blocks: {}", decorative_filter.exclude_falling);
    println!("• Exclude tile entities: {}", decorative_filter.exclude_tile_entities);
    println!("• Full blocks only: {}", decorative_filter.full_blocks_only);
    println!("• Exclude needs support: {}", decorative_filter.exclude_needs_support);
    println!("• Exclude transparent: {}", decorative_filter.exclude_transparent);
    println!("• Allows stairs, slabs, fences: Yes");
    println!();
    
    if let Some(rustic_decorative) = BlockPaletteGenerator::generate_architectural_palette_filtered("rustic", &decorative_filter) {
...
fn showcase_custom_filters() {
    ...
    if let Some(ocean_no_liquids) = BlockPaletteGenerator::generate_natural_palette_filtered("ocean", &no_liquids_filter) {
    ...
    if let Some(modern_concrete) = BlockPaletteGenerator::generate_architectural_palette_filtered("modern", &concrete_only_filter) {
    ...
    if let Some(nether_dark) = BlockPaletteGenerator::generate_natural_palette_filtered("nether", &no_light_filter) {
...
fn showcase_filter_comparison() {
    ...
    for (filter_name, filter) in filters {
        if let Some(palette) = BlockPaletteGenerator::generate_architectural_palette_filtered("medieval", &filter) {
    ...
    let solid_count = BLOCKS.values().filter(|b| solid_filter.allows_block(b)).count();
    let structural_count = BLOCKS.values().filter(|b| structural_filter.allows_block(b)).count();
    
    println!("📊 Block Filtering Statistics:");
    println!("   • Total blocks in game: {}", total_blocks);
    println!("   • Solid filter allows: {} blocks ({:.1}%)", 
        solid_count, (solid_count as f64 / total_blocks as f64) * 100.0);
    println!("   • Structural filter allows: {} blocks ({:.1}%)", 
        structural_count, (structural_count as f64 / total_blocks as f64) * 100.0);
    println!();
