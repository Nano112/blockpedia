...
mod data_sources_build {
    ...
    impl DataSourceRegistry {
        ...
        fn fetch_with_fallback(&self, primary: &dyn DataSourceAdapter) -> Result<Vec<UnifiedBlockData>> {
            // Try primary source
            match self.try_fetch_source(primary) {
                Ok(blocks) => {
                    println!("cargo:warning=Successfully fetched {} blocks from {}", blocks.len(), primary.name());
                    return Ok(blocks);
                }
                Err(e) => {
                    println!("cargo:warning=Failed to fetch from {} ({})", primary.name(), e);
                }
            }
            ...
            for source in &self.sources {
                if source.name() != primary.name() {
                    match self.try_fetch_source(source.as_ref()) {
                        Ok(blocks) => {
                            println!("cargo:warning=Successfully fell back to {} and fetched {} blocks", source.name(), blocks.len());
                            return Ok(blocks);
                        }
                        Err(e) => {
                            println!("cargo:warning=Fallback to {} also failed: {}", source.name(), e);
                        }
                    }
                }
            }
        ...
        fn try_fetch_source(&self, source: &dyn DataSourceAdapter) -> Result<Vec<UnifiedBlockData>> {
            let url = source.fetch_url();
            println!("cargo:warning=Fetching data from {} using {}", url, source.name());
            ...
            if cache_path.exists() {
                ...
                println!("cargo:warning=Cache invalid for {}, re-downloading", source.name());
            }
            ...
            source.validate_structure(&parsed)
                .with_context(|| format!("Data validation failed for {}", source.name()))?;
            
            // Parse into unified format
            let blocks = source.parse_data(&json_data)
                .with_context(|| format!("Failed to parse data from {}", source.name()))?;
            
            // Cache the successful data
            if let Err(e) = std::fs::write(&cache_path, &json_data) {
                println!("cargo:warning=Failed to cache data for {}: {}", source.name(), e);
            } else {
                println!("cargo:warning=Cached data for {} for future builds", source.name());
            }
...
impl FetcherRegistry {
    ...
    fn add_color_data(&mut self, block_id: &str, rgb: (u8, u8, u8)) {
        ...
        self.extra_data.color_data.insert(block_id.to_string(), (rgb.0, rgb.1, rgb.2, l, a, b_val));
    }
    ...
    fn extract_colors_from_textures(&mut self, available_block_ids: &[String]) -> Result<()> {
                ...
                match self.extract_color_from_texture(&texture_path) {
                    ...
                    Err(e) => {
                        ...
                        if failed_count <= 5 { // Only show first few errors
                            println!("cargo:warning=Failed to extract color from {}: {}", texture_name, e);
                        }
                    }
                }
            }
        ...
        println!("cargo:warning=Color extraction complete: {} colors extracted, {} failures", extracted_count, failed_count);
        Ok(())
    }
    ...
    fn add_inherited_colors(&mut self, available_block_ids: &[String]) {
        ...
        println!("cargo:warning=Color inheritance complete: {} colors inherited from base materials", inherited_count);
    }
    ...
    fn get_base_material_for_block(&self, block_id: &str) -> Option<String> {
        ...
        if block_name.ends_with("_slab") {
            ...
            match base.as_str() {
                ...
                "waxed_weathered_cut_copper" => return Some("minecraft:weathered_copper".to_string()),
                "waxed_oxidized_cut_copper" => return Some("minecraft:oxidized_copper".to_string()),
                ...
                "polished_blackstone_brick" => return Some("minecraft:polished_blackstone_bricks".to_string()),
    ...
    fn fetch_all(&mut self, available_block_ids: &[String]) -> Result<&ExtraData> {
        // Mock fetcher data
        self.extra_data.mock_data.insert("minecraft:stone".to_string(), 42);
        self.extra_data.mock_data.insert("minecraft:dirt".to_string(), 123);
        self.extra_data.mock_data.insert("minecraft:grass_block".to_string(), 456);
        self.extra_data.mock_data.insert("minecraft:oak_log".to_string(), 789);
        self.extra_data.mock_data.insert("minecraft:oak_planks".to_string(), 321);
        self.extra_data.mock_data.insert("minecraft:cobblestone".to_string(), 654);
        ...
        if let Err(e) = self.extract_colors_from_textures(available_block_ids) {
            println!("cargo:warning=Failed to extract colors from textures: {}", e);
        }
...
fn main() -> Result<()> {
    ...
    if let Ok(source_name) = env::var("BLOCKPEDIA_DATA_SOURCE") {
        println!("cargo:warning=Setting data source to {} from environment variable", source_name);
        data_registry.set_primary_source(&source_name)
            .with_context(|| format!("Failed to set data source to {}", source_name))?;
    }
    
    println!("cargo:warning=Available data sources: {:?}", data_registry.list_sources());
    println!("cargo:warning=Using primary data source: {}", data_registry.get_primary_source()?.name());
    ...
    let unified_blocks = if env::var("BLOCKPEDIA_USE_TEST_DATA").is_ok() {
        ...
        match data_registry.fetch_unified_data() {
            Ok(blocks) => blocks,
            Err(e) => {
                println!("cargo:warning=Failed to fetch from primary source ({}): {}", data_registry.get_primary_source()?.name(), e);
...
fn fetch_or_load_cached(cache_path: &Path) -> Result<String> {
    ...
    if std::env::var("BLOCKPEDIA_USE_TEST_DATA").is_ok() {
        ...
        if test_file.exists() {
            ...
            return fs::read_to_string(test_file)
                .context("Failed to read test JSON file");
        }
    }
    ...
    if cache_path.exists() {
        ...
        return fs::read_to_string(cache_path)
            .context("Failed to read cached JSON file");
    }
    ...
    match download_json() {
        Ok(data) => {
            ...
            fs::write(cache_path, &data)
                .context("Failed to cache downloaded JSON")?;
            Ok(data)
        }
        Err(e) => {
            anyhow::bail!("Failed to download blocks_data.json and no cache available: {}", e);
        }
    }
}

fn download_json() -> Result<String> {
    let response = reqwest::blocking::get(BLOCKS_DATA_URL)
        .context("Failed to make HTTP request")?;
...
fn validate_json_structure(json: &Value) -> Result<()> {
    ...
    if json.is_object() && json.get("blocks").is_some() {
        ...
        let blocks = json["blocks"].as_object()
            .context("'blocks' field is not an object")?;
        ...
        println!("cargo:warning=JSON validation passed - found {} blocks (test format)", blocks.len());
    } else if json.is_array() {
        // PrismarineJS format: [{"name": "air", ...}, ...]
        let blocks_array = json.as_array()
            .context("JSON is not a valid array")?;
        ...
        println!("cargo:warning=JSON validation passed - found {} blocks (PrismarineJS format)", blocks_array.len());
    } else {
        anyhow::bail!("JSON format not recognized - expected either {{\"blocks\": {{...}}}} or array format");
    }
    
    Ok(())
}

fn generate_phf_table(out_dir: &str, json: &Value, extra_data: &ExtraData, fetcher_registry: &FetcherRegistry) -> Result<()> {
    let table_path = Path::new(out_dir).join("block_table.rs");
    let mut file = std::fs::File::create(&table_path)
        .context("Failed to create block_table.rs")?;
    ...
    let block_data: Vec<(String, serde_json::Value)> = if json.is_object() && json.get("blocks").is_some() {
        // Test format: {"blocks": {"minecraft:stone": {...}, ...}}
        let blocks_obj = json["blocks"].as_object()
            .context("'blocks' field is not an object")?;
        blocks_obj.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
    } else if json.is_array() {
        // PrismarineJS format: [{"name": "air", ...}, ...]
        let blocks_array = json.as_array()
            .context("JSON is not a valid array")?;
        
        blocks_array.iter().enumerate().filter_map(|(_i, block)| {
            let block_obj = block.as_object()?;
            ...
            if let Some(states) = block_obj.get("states").and_then(|s| s.as_array()) {
                ...
                for state in states {
                    if let Some(state_obj) = state.as_object() {
                        if let (Some(prop_name), Some(prop_type), Some(num_values)) = (
                            ...
                            properties.insert(prop_name.to_string(), serde_json::Value::Array(
                ...
                if !properties.is_empty() {
                    converted_block.insert("properties".to_string(), serde_json::Value::Object(properties));
                } else {
                    converted_block.insert("properties".to_string(), serde_json::Value::Object(serde_json::Map::new()));
                }
            } else {
                converted_block.insert("properties".to_string(), serde_json::Value::Object(serde_json::Map::new()));
            }
            ...
            converted_block.insert("default_state".to_string(), serde_json::Value::Object(serde_json::Map::new()));
            
            Some((format!("minecraft:{}", name), serde_json::Value::Object(converted_block)))
        }).collect()
    } else {
    ...
    for (block_id, block_data) in &block_data {
        ...
        let properties = block_obj.get("properties")
            .and_then(|p| p.as_object())
            .unwrap_or(&empty_props);
        
        // Parse default state
        let empty_state = serde_json::Map::new();
        let default_state = block_obj.get("default_state")
        ...
        writeln!(file, "static {}: crate::BlockFacts = crate::BlockFacts {{", safe_name)?;
        ...
        for (state_name, state_value) in default_state {
            writeln!(file, "        (\"{}\", \"{}\"),", state_name, state_value.as_str().unwrap_or(""))?;
        }
        writeln!(file, "    ],")?;
    ...
    writeln!(file, "pub static BLOCKS: Map<&'static str, &'static crate::BlockFacts> = phf_map! {{")?;
    ...
    println!("cargo:warning=Generated PHF table with {} blocks", block_data.len());
    Ok(())
}
...
fn generate_unified_phf_table(out_dir: &str, unified_blocks: &[UnifiedBlockData]) -> Result<()> {
    ...
    for block_data in unified_blocks {
        ...
        // Generate a valid Rust identifier from block ID
        let safe_name = block_id.replace(":", "_")
            .replace("-", "_")
            .replace("'", "")
            .replace("!", "")
            .replace(".", "_")
    ...
    println!("cargo:warning=Generated unified PHF table with {} blocks", unified_blocks.len());
    Ok(())
}
