...
fn main() {
    ...
    for block_id in &blocks_without_color {
        ...
        categories.entry(category).or_insert(Vec::new()).push(block_id);
    }
    ...
    for (category, blocks) in &categories {
        ...
        println!("  {:20} | {:3} blocks ({:4.1}%)", category, blocks.len(), percentage);
    }
    ...
    if let Some(redstone_blocks) = categories.get("Redstone") {
        println!("  ⚡ Redstone blocks ({} blocks): Need redstone-specific textures", redstone_blocks.len());
    }
    ...
    if let Some(technical_blocks) = categories.get("Technical/Special") {
        println!("  🔧 Technical blocks ({} blocks): May need custom textures or computed colors", technical_blocks.len());
    }
    
    if let Some(stairs_blocks) = categories.get("Stairs") {
        println!("  🪜 Stairs ({} blocks): Could inherit colors from base block materials", stairs_blocks.len());
    }
    
    if let Some(slab_blocks) = categories.get("Slabs") {
        println!("  📦 Slabs ({} blocks): Could inherit colors from base block materials", slab_blocks.len());
    }
    
    if let Some(wall_blocks) = categories.get("Walls") {
        println!("  🧱 Walls ({} blocks): Could inherit colors from base block materials", wall_blocks.len());
    }
...
fn categorize_block(block_id: &str) -> &'static str {
    ...
    match block_name {
        ...
        // Redstone components
        name if name.contains("redstone") || name.contains("repeater") || name.contains("comparator") ||
                name.contains("lever") || name.contains("button") || name.contains("pressure_plate") ||
                name.contains("tripwire") || name.contains("piston") || name.contains("dispenser") ||
                name.contains("dropper") || name.contains("hopper") => "Redstone",
        
        // Doors and gates
        name if name.ends_with("_door") || name.ends_with("_gate") || name.ends_with("_trapdoor") => "Doors/Gates",
        ...
        // Plants and natural
        name if name.contains("flower") || name.contains("grass") || name.contains("fern") ||
                name.contains("vine") || name.contains("leaves") || name.contains("sapling") ||
                name.contains("kelp") || name.contains("seagrass") || name.contains("coral") ||
                ...
                name.contains("beetroot") || name.contains("melon_stem") || name.contains("pumpkin_stem") ||
                name.contains("sweet_berry") || name.contains("cocoa") || name.contains("mushroom") ||
                name.contains("fungus") || name.contains("roots") || name.contains("lily_pad") => "Plants/Natural",
        ...
        name if name.contains("candle") || name.contains("torch") || name.contains("lantern") ||
                name.contains("soul_fire") || name.contains("fire") => "Lighting",
        ...
        name if name.contains("command") || name.contains("structure") || name.contains("jigsaw") ||
                name.contains("debug") || name.contains("end_portal") || name.contains("nether_portal") ||
                name.contains("moving_piston") || name.contains("piston_head") => "Technical/Special",
        
        // Crops and farmland
        name if name.contains("crop") || name.contains("farmland") || name.contains("soil") => "Farming",
