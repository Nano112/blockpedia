...
use ratatui::widgets::{
    Block, Borders, List, ListItem, Paragraph, Tabs, Table, Row, Cell, BarChart
};
use ratatui::layout::{Layout, Constraint, Direction, Alignment, Rect};
use ratatui::style::{Color, Style, Modifier};
use ratatui::text::Line;
use blockpedia::{BLOCKS, BlockFacts, queries::*};
...
fn ui(f: &mut ratatui::Frame, app: &App) {
    ...
    let titles = ["📦 Blocks", "🎨 Colors", "🔍 Query", "⚙️ Properties", "📊 Stats", "🌐 Sources"]
        .iter()
        .cloned()
        .map(Line::from)
        .collect();
    ...
    let inner_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints::<&[ratatui::layout::Constraint]>([Constraint::Length(3), Constraint::Min(0)].as_ref())
        .split(chunks[1]);
...
fn render_blocks_tab(f: &mut ratatui::Frame, app: &App, area: Rect) {
    let chunks = Layout::default()
        ...
        .constraints::<&[ratatui::layout::Constraint]>([Constraint::Percentage(50), Constraint::Percentage(50)].as_ref())
        .split(area);

    // Block list
    let items: Vec<ListItem> = app.filtered_blocks
        .iter()
        .enumerate()
        .map(|(i, block)| {
    ...
    let blocks_list = List::new(items)
        .block(Block::default().borders(Borders::ALL).title(format!("Blocks ({}/{})", 
            app.filtered_blocks.len(), BLOCKS.len())))
...
fn render_colors_tab(f: &mut ratatui::Frame, app: &App, area: Rect) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints::<&[ratatui::layout::Constraint]>([Constraint::Length(10), Constraint::Min(0)].as_ref())
        .split(area);
    ...
    let options_paragraph = Paragraph::new(options_text)
        .block(Block::default().borders(Borders::ALL).title("🎨 Color Queries"));
    f.render_widget(options_paragraph, chunks[0]);

    // Color query results
    let results_text: Vec<Line> = app.query_results.iter().map(|s| Line::from(s.clone())).collect();
...
fn render_query_tab(f: &mut ratatui::Frame, app: &App, area: Rect) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints::<&[ratatui::layout::Constraint]>([Constraint::Length(8), Constraint::Min(0)].as_ref())
        .split(area);
    ...
    if app.input_mode == InputMode::Searching {
        ...
        display_options.push(format!("✏️  Type your {} query:", match app.search_mode {
    ...
    let options_text: Vec<Line> = display_options.iter().map(|s| Line::from(s.clone())).collect();
    let options_paragraph = Paragraph::new(options_text)
        .block(Block::default().borders(Borders::ALL).title("Query Options"));
    f.render_widget(options_paragraph, chunks[0]);
...
fn render_properties_tab(f: &mut ratatui::Frame, _app: &App, area: Rect) {
    ...
    let rows: Vec<Row> = all_properties.iter().map(|(prop, values)| {
        Row::new(vec![
    ...
    let widths = [Constraint::Length(15), Constraint::Length(8), Constraint::Min(20)];
    let table = Table::new(rows, widths)
        .header(Row::new(vec!["Property", "Count", "Values"]).style(Style::default().fg(Color::Yellow)))
        .block(Block::default().borders(Borders::ALL).title("All Properties"));
...
fn render_statistics_tab(f: &mut ratatui::Frame, _app: &App, area: Rect) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints::<&[ratatui::layout::Constraint]>([Constraint::Length(6), Constraint::Min(0)].as_ref())
        .split(area);
    ...
    let stats_text = vec![
        ...
        format!("Blocks with no properties: {}", stats.blocks_with_no_properties),
        format!("Average properties per block: {:.2}", stats.average_properties_per_block),
    ];
    ...
    let property_counts: Vec<_> = all_props.iter()
        .map(|(prop, values)| (prop.as_str(), values.len() as u64))
        .take(10)
        .collect();
    
    let barchart = BarChart::default()
        .block(Block::default().borders(Borders::ALL).title("Property Value Counts (Top 10)"))
...
impl App {
    ...
    fn next_block(&mut self) {
        if !self.filtered_blocks.is_empty() {
            self.selected_block_index = (self.selected_block_index + 1) % self.filtered_blocks.len();
        }
    }
    ...
    fn run_pattern_search(&mut self) {
        ...
        if results.len() > 10 {
            self.query_results.push(format!("  ... and {} more", results.len() - 10));
        }
    }
    ...
    fn run_rare_properties_query(&mut self) {
        ...
        for (prop, count) in rare_props {
            self.query_results.push(format!("  - {}: {} blocks", prop, count));
        }
    }
    ...
    fn run_families_query(&mut self) {
        ...
        for (family, blocks) in families {
            self.query_results.push(format!("  - {}: {} blocks", family, blocks.len()));
        }
    }
    ...
    fn run_statistics_query(&mut self) {
        ...
        self.query_results = vec![
            ...
            format!("Most common property: {} ({} blocks)", 
    ...
    fn run_color_coverage_query(&mut self) {
        ...
        let blocks_with_color: Vec<_> = BLOCKS.values().filter(|b| b.extras.color.is_some()).collect();
        ...
        for block in blocks_with_color.iter().take(10) {
            if let Some(color) = &block.extras.color {
                ...
                self.query_results.push(format!("  • {} → {}", block.id(), hex));
            }
        }
        
        if blocks_with_color.len() > 10 {
            self.query_results.push(format!("  ... and {} more", blocks_with_color.len() - 10));
        }
    }
    
    fn run_color_palette_query(&mut self) {
        let colored_blocks: Vec<_> = BLOCKS.values().filter(|b| b.extras.color.is_some()).collect();
    ...
    fn run_color_similarity_query(&mut self) {
        ...
        for block in colored_blocks {
            if let Some(color) = &block.extras.color {
                ...
                if distance < 50.0 { // Threshold for similarity
        ...
        for (block, color, distance) in similar_blocks.iter().take(10) {
            let hex = format!("#{:02X}{:02X}{:02X}", color.rgb[0], color.rgb[1], color.rgb[2]);
            self.query_results.push(
                format!("  • {} → {} (distance: {:.1})", block.id(), hex, distance)
            );
        }
    }
    ...
    fn run_color_analysis_query(&mut self) {
        ...
        self.query_results = vec![
            ...
            format!("  #{:02X}{:02X}{:02X}", avg_red as u8, avg_green as u8, avg_blue as u8),
            "".to_string(),
            format!("Brightest block: {} (brightness: {})", brightest.0, brightest.1),
    ...
    fn run_gradient_palette_query(&mut self) {
        ...
        let stone_block = colored_blocks.iter().find(|b| b.id().contains("stone")).unwrap_or(&colored_blocks[0]);
        let grass_block = colored_blocks.iter().find(|b| b.id().contains("grass")).unwrap_or(&colored_blocks[1]);
        ...
        let gradient = PaletteGenerator::generate_block_gradient_palette(
            stone_color, grass_color, 10, GradientMethod::LinearOklab
        );
    ...
    fn run_themed_palette_query(&mut self) {
        ...
        self.query_results.push("🌊 Ocean Depths Palette (6 colors):".to_string());
        ...
        self.query_results.push("".to_string());
        self.query_results.push("🔥 Fire Palette (5 colors):".to_string());
    ...
    fn run_block_palette_query(&mut self) {
        ...
        if let Some(gradient_palette) = BlockPaletteGenerator::generate_block_gradient(stone_block, grass_block, 5) {
            self.query_results.push(format!("🌈 {} ({})", gradient_palette.name, gradient_palette.blocks.len()));
            self.query_results.push(gradient_palette.description);
            self.query_results.push("".to_string());
            
            for (i, rec) in gradient_palette.blocks.iter().enumerate() {
                let block_name = rec.block.id().strip_prefix("minecraft:").unwrap_or(rec.block.id());
        ...
        if let Some(mono_palette) = BlockPaletteGenerator::generate_monochrome_palette(stone_block, 7) {
            self.query_results.push("".to_string());
            self.query_results.push(format!("⚫ {} ({})", mono_palette.name, mono_palette.blocks.len()));
    ...
    fn run_architectural_palette_query(&mut self) {
        ...
        for style in &styles {
            if let Some(palette) = BlockPaletteGenerator::generate_architectural_palette(style) {
                ...
                self.query_results.push(format!("{} {} ({})", icon, palette.name, palette.blocks.len()));
                ...
                for (_i, rec) in palette.blocks.iter().take(4).enumerate() {
                    ...
                    self.query_results.push(format!(
                        ...
                        block_name.replace('_', " "),
                        match rec.role {
                            blockpedia::color::block_palettes::BlockRole::Primary => "Main structure",
        ...
        if let Some(forest_palette) = BlockPaletteGenerator::generate_natural_palette("forest") {
            self.query_results.push("🌲 Forest Biome Palette".to_string());
            self.query_results.push(forest_palette.description);
            
            for rec in forest_palette.blocks.iter().take(3) {
                let block_name = rec.block.id().strip_prefix("minecraft:").unwrap_or(rec.block.id());
                self.query_results.push(format!("  • {} {}", rec.color.hex_string(), block_name.replace('_', " ")));
            }
        }
    ...
    fn search_by_name(&mut self) {
        ...
        let results: Vec<_> = BLOCKS.values()
        ...
        for block in self.filtered_blocks.iter().take(20) {
            ...
            self.query_results.push(format!("  • {}{}", block.id(), indicator_text));
        }
        
        if self.filtered_blocks.len() > 20 {
            self.query_results.push(format!("  ... and {} more", self.filtered_blocks.len() - 20));
        }
    }
    ...
    fn search_by_property(&mut self) {
        ...
        for block in self.filtered_blocks.iter().take(15) {
            ...
            self.query_results.push(format!("  • {}", details.join(" ")));
        }
        
        if self.filtered_blocks.len() > 15 {
            self.query_results.push(format!("  ... and {} more", self.filtered_blocks.len() - 15));
        }
    }
    ...
    fn search_by_color(&mut self) {
        ...
        self.filtered_blocks = similar_blocks.iter().take(50).map(|(block, _, _)| **block).collect();
        self.selected_block_index = 0;
        
        self.query_results = vec![
            format!("🎨 Color Search: {} → RGB({}, {}, {})", hex, target_r, target_g, target_b),
            "".to_string(),
            format!("Found {} similar blocks:", self.filtered_blocks.len()),
        ];
        
        for (block, color, distance) in similar_blocks.iter().take(15) {
            let block_hex = format!("#{:02X}{:02X}{:02X}", color.rgb[0], color.rgb[1], color.rgb[2]);
            self.query_results.push(
                format!("  • {} → {} (Δ {:.1})", block.id(), block_hex, distance)
            );
        }
        
        if similar_blocks.len() > 15 {
            self.query_results.push(format!("  ... and {} more", similar_blocks.len() - 15));
        }
    }
    ...
    fn execute_advanced_query(&mut self) {
        ...
        if !self.query_builder.name_pattern.is_empty() {
            self.query_results.push(format!("  • Name contains: '{}'", self.query_builder.name_pattern));
        }
        ...
        for filter in &self.query_builder.property_filters {
            ...
            self.query_results.push(format!("  • Property: {} {}{}", filter.property, op_text, value_text));
        }
        
        if let Some(color_filter) = &self.query_builder.color_filter {
            self.query_results.push(format!("  • Color similar to: {} (±{:.0})", 
        ...
        if self.filtered_blocks.len() > 10 {
            self.query_results.push(format!("  ... and {} more", self.filtered_blocks.len() - 10));
        }
    }
    ...
    fn apply_property_filter(&self, block: &BlockFacts, filter: &PropertyFilter) -> bool {
        match filter.operator {
            FilterOperator::Exists => {
                block.properties.iter().any(|(name, _)| name == &filter.property)
            }
            FilterOperator::Equals => {
                if let Some((_, values)) = block.properties.iter().find(|(name, _)| name == &filter.property) {
...
fn render_sources_tab(f: &mut ratatui::Frame, _app: &App, area: Rect) {
    ...
    let source_paragraph = Paragraph::new(source_lines)
        .block(Block::default().borders(Borders::ALL).title("Source Information"));
    ...
    let stats_lines: Vec<Line> = enhanced_stats.iter().map(|s| Line::from(s.clone())).collect();
    let stats_paragraph = Paragraph::new(stats_lines)
        .block(Block::default().borders(Borders::ALL).title("Current Build Stats"))
        .wrap(ratatui::widgets::Wrap { trim: true });
