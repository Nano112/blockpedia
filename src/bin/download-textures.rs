...
async fn main() -> Result<()> {
    ...
    fs::create_dir_all(textures_dir)
        .context("Failed to create textures directory")?;
    
    // Get list of available textures
    let client = reqwest::Client::new();
    let url = "https://api.github.com/repos/1280px/hueblocks/contents/data/blocksets/blocks?ref=legacy";
...
async fn test_color_extraction(textures_dir: &Path) -> Result<()> {
    ...
    let test_blocks = ["stone.png", "dirt.png", "grass_block_top.png", "oak_log.png"];
    ...
    for block_name in test_blocks {
        ...
        if texture_path.exists() {
            match extract_dominant_color(&texture_path) {
                Ok(color) => {
                    println!("  {} -> RGB({}, {}, {}) | {} | HSL({:.0}°, {:.1}%, {:.1}%)", 
                        block_name,
                        color.rgb[0], color.rgb[1], color.rgb[2],
                        color.hex_string(),
                        color.hsl[0], color.hsl[1] * 100.0, color.hsl[2] * 100.0
                    );
                }
                Err(e) => {
