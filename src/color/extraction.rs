...
use super::{ExtendedColorData, extract_dominant_color_from_image};
...
impl ColorExtractor {
    ...
    fn extract_most_frequent_color(&self, img: &DynamicImage, bins: u8) -> Result<ExtendedColorData> {
        ...
        let most_frequent = color_counts.iter()
...
pub fn extract_block_color_variants(block_name: &str, texture_paths: &[&Path]) -> Result<ExtendedColorData> {
    ...
    Ok(ExtendedColorData::from_rgb(r_avg as u8, g_avg as u8, b_avg as u8))
}
