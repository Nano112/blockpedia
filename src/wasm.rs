...
#[cfg(feature = "wasm")]
// Remove unused import - we'll use the macro instead

#[cfg(feature = "wasm")]
use crate::{BLOCKS, query_builder::*, color::ExtendedColorData};
...
pub fn get_all_colored_blocks() -> JsValue {
    let colored_blocks: Vec<WasmBlockInfo> = BLOCKS.values()
        .filter_map(|block| {
...
pub fn generate_gradient_between_blocks(
    ...
    match (BLOCKS.get(start_block_id), BLOCKS.get(end_block_id)) {
        (Some(start_block), Some(end_block)) => {
            if let (Some(_start_color), Some(_end_color)) = (start_block.extras.color, end_block.extras.color) {
                ...
                let blocks: Vec<WasmBlockInfo> = gradient.collect()
                    .into_iter()
                    .filter_map(|block| {
                        if let Some(color) = block.extras.color {
                            let ext_color = color.to_extended();
        ...
        _ => {
            let result = WasmGradientResult {
                blocks: Vec::new(),
                success: false,
                error: Some(format!("Block(s) not found: {} or {}", start_block_id, end_block_id)),
            };
            serde_wasm_bindgen::to_value(&result).unwrap()
        }
    }
...
pub fn get_easing_functions() -> JsValue {
    let easing_functions = vec!["linear", "ease-in", "ease-out", "ease-in-out", "sine", "exponential"];
